...
class Worker(mp.Process):
    ...
    def train(self, done):
        ...
        # Sets the value function in terminal state to zero
        if done:
            values[-1] = 0

        # Initiate batch_return vector
        batch_return = []

        # Hyperparameters: Lambda and steps in return
        lamda = 0.5
        n = 5

        if n == len(self.memory.rewards): # If Monte Carlo don't use the
                                          # value function in return
            MC = True
        else:
            MC = False

        if n > len(self.memory.rewards):
            n = len(self.memory.rewards) # Keep in bounds in for-loop below

        for i in range(len(self.memory.rewards)): # For every step in training batch
            # Initial value for eligibility trace, reward and discounted return
            e = 1
            R = 0
            dis_return = 0
            for j in range(i, len(self.memory.rewards) - n - 1): # From i to last step in batch_return
                # Calculate TD-error, return and eligibility trace
                for k in range(n):
                    dis_return += (self.gamma ** (k + 1)) * self.memory.rewards[j + k]
                if MC: # If MC don't use value function in return
                    delta = dis_return - values[j]
                else:
                    delta = dis_return + (self.gamma ** (n + 1)) * values[j + n + 1] - values[j]
                R = e * delta + R
                e = self.gamma * lamda * e
            # Append return to batch vector
